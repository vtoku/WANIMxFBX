...
- **Body mesh** (`src/convert/body.ts` + `public/body.glb` = Quaternius "Animated Base Character", CC0, textures stripped): a 4-step transfer that works for ANY humanoid GLB — (1) map source bone names → our bones (`RIGIFY_MAP` for Blender DEF-* rigs + MoBu names for Mixamo rigs; unmapped helpers walk up to the nearest mapped ancestor), (2) **FK-align the source rest pose to OUR T-pose** (rotate each bone so its segment direction matches ours — REQUIRED: real assets ship A-poses/idle rests and arbitrary facings), (3) evaluate the skin CPU-side via `SkinnedMesh.applyBoneTransform` (do NOT trust inverse-bind matrices for joint positions — Blender glTF exports ship near-identity inverse binds with the bind pose living in node rest transforms; Mixamo exports bake armature scale into them), (4) per-vertex transfer with axis-only SEGMENT stretch (girth preserved). Head/eye triangles are dropped (the Face mesh replaces them); normals recomputed after transfer. The "Proportions: Body-mesh skeleton" option re-proportions the CLIP onto the body's joints (`retargetProportions`) for 1:1 mapping.
